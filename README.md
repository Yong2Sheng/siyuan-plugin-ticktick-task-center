...
思源笔记滴答任务中心是一个思源笔记插件，用于把普通思源文档块关联到 TickTick / 滴答清单任务。原文档中的任务块是唯一持久化数据源；插件在其上提供任务卡片增强、七种本地状态、每日推进记录和全工作空间任务中心，不会维护第二套任务数据库。
...
- 在一个普通思源块中保存任务标题、经过校验的 TickTick URL、七项必需属性，以及可选的每日推进日期属性。
...
- 显示全部、活跃、已结束和今日进度统计，并按任务属性中的 `updated-at` 降序稳定排列。
...
在任务中心编辑或记录今日推进后立即点击刷新时，思源 SQL 索引可能暂时仍返回旧值。插件会在当前任务中心页签内保留最近编辑或推进结果，避免界面退回旧标题、URL、状态或推进分组；当 SQL 返回对应数据，或该任务不再有效时，这个临时覆盖会被清除。它不会替代任务块属性，不会跨页签持久化，也不是第二套任务数据库。
...
老任务没有该属性时直接视为今日待推进，无需迁移。插件只比较保存日期与系统本地日期，不在午夜批量改写任务块。

## 任务中心查询

任务中心每次加载或手动刷新时执行一次全局 SQL 查询。七项必需任务属性和可选每日推进属性通过条件聚合转换为每个任务一行，避免旧式“一项属性一行”的结果在思源 SQL 查询数量上限处产生属性不完整的半个任务。
