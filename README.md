...
思源笔记滴答任务中心是一个思源笔记插件，用于把普通思源文档块关联到 TickTick / 滴答清单任务。原文档中的任务块是唯一持久化数据源；插件在其上提供任务卡片增强、七种本地状态、截止日期提醒、每日推进记录和全工作空间任务中心，不会维护第二套任务数据库。
...
- 在一个普通思源块中保存任务标题、经过校验的 TickTick URL、七项必需属性，以及可选的截止日期和每日推进日期属性。
- 在原始块中保留包含标题和链接的普通 Markdown；插件关闭时仍可阅读和打开链接。
- 在文档内非破坏性地增强显示任务卡片，包括任务链接、插件自有清单图标、带语义颜色的状态徽章和 Deadline 按钮。
- 点击状态徽章或 Deadline 按钮打开完整编辑窗口，可修改标题、URL、状态和截止日期。
...
- 完成任务卡片创建与编辑、七种本地状态、截止日期和每日推进记录。
...
7. 根据需要设置截止日期；该字段可以留空，也可以稍后补充或清除。
8. 点击“创建”。
9. 新任务会作为当前根文档的第一个子块插入，也就是位于文档标题下方、其他正文之前，并立即增强显示为任务卡片。

### 2. 在文档中编辑任务

- 点击任务卡片右侧的状态徽章，打开完整编辑窗口并聚焦状态字段。
- 点击最右侧的 Deadline 按钮，打开同一个编辑窗口并聚焦截止日期字段。
- 修改任务标题、TickTick URL、任务状态或截止日期，然后保存。
- 保存成功后，当前文档中的卡片立即更新。
...
老任务没有这些可选属性时无需迁移：没有每日推进日期时直接视为今日待推进，没有截止日期时显示“无截止日期”并在 Deadline 排序中置后。插件只比较保存日期与系统本地日期，不在午夜批量改写任务块。

## 任务中心查询

任务中心每次加载或手动刷新时执行一次全局 SQL 查询。七项必需任务属性以及可选的每日推进和截止日期属性通过条件聚合转换为每个任务一行，避免旧式“一项属性一行”的结果在思源 SQL 查询数量上限处产生属性不完整的半个任务。
